...
class UserNotFound(DomainException):
    def __init__(self, identifier: str = ""):
        super().__init__(
            f"User not found: {identifier}" if identifier else "User not found"
        )
...
class ServiceNotFound(DomainException):
    def __init__(self, identifier: str = ""):
        super().__init__(
            f"Service not found: {identifier}" if identifier else "Service not found"
        )
...
class BookingNotFound(DomainException):
    def __init__(self, identifier: str = ""):
        super().__init__(
            f"Booking not found: {identifier}" if identifier else "Booking not found"
        )
...
class BookingConflict(DomainException):
    def __init__(self) -> None:
        super().__init__(
            "A booking already exists for this service at the requested time"
        )
...
class InvalidEmailFormat(DomainException):
    def __init__(self, email: str = "") -> None:
        super().__init__(
            f"Invalid email format: {email}" if email else "Invalid email format"
        )
